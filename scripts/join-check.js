 ...
 * value movement signed by a self-custodial WDK wallet (sendTransaction +
 * calldata — no ethers signers anywhere in the journey).
 *
...
async function send(account, to, data, label) {
  const { hash, fee } = await account.sendTransaction({ to, data });
...
await send(creator, cfg.usdtContract, erc20.encodeFunctionData("approve", [cfg.escrowContract, stake]), "creator approve");
await send(creator, cfg.escrowContract, escrowAbi.encodeFunctionData("create", [betId, stake, jurors, deadline]), "creator create");
await send(joiner, cfg.usdtContract, erc20.encodeFunctionData("approve", [cfg.escrowContract, stake]), "joiner approve ");
